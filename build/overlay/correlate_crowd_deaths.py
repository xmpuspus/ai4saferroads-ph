...
def perm_p(x, y, observed, k=4999):
    ...
    n = len(y)
    cnt = sum(
        abs(spearman(x, np.roll(y, (sh * 7919) % n))) >= abs(observed) - 1e-12
...
def main():
    ...
    for k in keys:
        ...
        r = per_region.setdefault(
            reg, {"flag_dens": [], "flag_gap": [], "n_flag": 0, "cities": []}
        )
        r["flag_dens"] += fd
        r["flag_gap"] += fg
        r["n_flag"] += len(fd)
    ...
    seg = {
        "n_flagged_realposted": len(flag_d),
        "n_unflagged_realposted": len(unflag_d),
    ...
    pooled = {
        "n_cells": len(cg),
        "rho_gap_density": round(rho_cell, 3),
        "p": round(p_cell, 4),
    }
    ...
    dens_all = np.array(
        [d for _, d in [(g, d) for k in keys for g, d in (load_city(k)[0] or [])]]
    )
        ...
        hooks[k] = [
            {
                "road": x[0],
                "gap_kmh": x[1],
                "density_km2": round(x[2], 0),
                "posted": x[3],
                "safe": x[4],
                "sss": x[5],
                "lat": x[6],
                "lon": x[7],
                "density_peak_km2": round(x[8], 0),
            }
            for x in top
        ]

    result = {
        "segment_crowding": seg,
        "pooled_cell": pooled,
        "density_decile90_km2": round(decile9, 0),
        "triple_region": triple,
        "regions": regions,
    ...
    print(
        f"  flagged median density   {seg['median_density_flagged']:>8.0f}/km2  (n={seg['n_flagged_realposted']:,})"
    )
    print(
        f"  unflagged median density {seg['median_density_unflagged']:>8.0f}/km2  (n={seg['n_unflagged_realposted']:,})"
    )
    print(
        f"  P(flagged denser than unflagged) = {seg['prob_flagged_denser_than_unflagged']}"
    )
    print(
        f"=== 2. POOLED CELL gap~density: rho={pooled['rho_gap_density']:+.3f} p={pooled['p']:.4f} (n={pooled['n_cells']}) ==="
    )
    print(f"=== 3. TRIPLE (region, n={triple['n_regions']}) ===")
    print(
        f"  flagged-road density vs death rate: rho={triple['rho_flagdensity_vs_deathrate']:+.3f} p={triple['p_flagdensity_vs_deathrate']:.3f}"
    )
    print(
        f"  n flagged roads     vs death rate: rho={triple['rho_nflagged_vs_deathrate']:+.3f} p={triple['p_nflagged_vs_deathrate']:.3f}"
    )
    print("  region                 cities nflag  medDens   gap  rate22  cnt23")
    for r in regions:
        print(
            f"  {r['name']:20s} {r['cities']:4d} {r['n_flagged']:6d} {r['median_flag_density']:8.0f} {r['mean_gap']:5.1f} {str(r['death_rate_2022']):>6} {str(r['death_count_2023']):>6}"
        )
    print(
        f"=== 4. FAST ROAD THROUGH DENSEST HOUSING (top-decile density>= {decile9:.0f}/km2) ==="
    )
    for k, lst in hooks.items():
        if lst:
            print(f"  {k}:")
            for h in lst:
                print(
                    f"     {h['road'][:34]:34s} gap {h['gap_kmh']:+3d} ({h['posted']}->{h['safe']}) dens {h['density_km2']:.0f}/km2 @ {h['lat']},{h['lon']}"
                )
