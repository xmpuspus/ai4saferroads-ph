...
def main():
    ...
    for k in CITIES:
        ...
        rows.append({
            "label": CITIES[k]["label"], "scored": s["segments_total"],
            "real": s["segments_posted_real_osm"],
            "flag": s["segments_flagged_real_posted"], "vru": s["vru_pois"],
            "rho": rho, "p": pv})
        for key, col in (("scored", "segments_total"), ("real", "segments_posted_real_osm"),
                         ("flag", "segments_flagged_real_posted"), ("vru", "vru_pois")):
            tot[key] += s[col]
    ...
    for r in rows:
        lines.append(f"| {r['label']} | {fmt(r['scored'])} | {fmt(r['real'])} | "
                     f"{fmt(r['flag'])} | {fmt(r['vru'])} | {rp(r)} |")
    lines.append(f"| **All 51 cities** | **{fmt(tot['scored'])}** | **{fmt(tot['real'])}** | "
                 f"**{fmt(tot['flag'])}** | **{fmt(tot['vru'])}** | | |")
    lines.append("")
    (DOCS / "cities.md").write_text("\n".join(lines))
    print(f"wrote docs/cities.md: {len(rows)} cities, flagged total {tot['flag']:,}, "
          f"VRU {tot['vru']:,}")
